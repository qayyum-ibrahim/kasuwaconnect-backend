...
const aiClient = axios.create({
  ...
  timeout: 30000, // 15 second timeout — Render free tier can be slow to wake
  ...
  try {
    ...
    const payload = {
      ...
      avg_daily_transactions:
        trader.totalTransactions / Math.max(monthsActive * 30, 1),
      avg_transaction_amount: avgTransactionAmount,
      trade_days_per_week: Math.min(
        trader.totalTransactions / Math.max(monthsActive * 4.3, 1),
        7,
      ),
      supplier_diversity: Math.min(
        Math.floor(trader.totalTransactions / 10),
        15,
      ),
      payment_regularity:
        trader.totalTransactions > 0
